...
const CertificationsCRUD = () => {
  ...
  }, [certifications, debouncedSearch, sortOrder]);

  // HANDLERS
  const handleOpenModal = (certification = null) => {
    if (certification) {
      setEditingId(certification.id);
      setFormData({ name: certification.name });
      setExistingImage(certification.image_url);
    } else {
      setEditingId(null);
      setFormData({ name: '' });
      setExistingImage('');
  ...
  const columns = [
    {
      ...
      render: (item) => (
        <img 
          ...
          alt={item.name}
          className="w-12 h-12 rounded object-cover border-2 border-green-300"
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 bg-[#F4F2EE] min-h-screen">
      ...
      <AdminTable
        ...
        columns={columns}
        loading={loading && certifications.length === 0}
        ...
        <form onSubmit={handleSave} className="space-y-6">
          <div>
            ...
            <input
              ...
              placeholder="e.g. ISO 9001, FSC Certified"
            />
          </div>
