...
const Users = () => {
  ...
  const handleOpenModal = (user = null) => {
    if (user) {
      ...
      setEditingId(null);
      setFormData({ name: '', email: '', password: '', division: '', role_id: '' });
    }
    setShowModal(true);
  };
  ...
  const handleSave = async (e) => {
    ...
    if (!formData.name.trim() || !formData.email.trim() || (!editingId && !formData.password)) return;
  ...
  const columns = [
    {
      ...
      render: (item) => (
        <div className="flex flex-col">
          ...
          <span className="text-[10px] text-gray-400 font-medium tracking-wider">{item.email}</span>
        </div>
      )
    },
    {
    ...
    },
    {
      ...
      render: (item) => (
        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${
          item.role?.name === 'Super Admin' ? 'bg-[#3C2F26] text-white' : 'bg-blue-50 text-blue-600 border border-blue-100'
        }`}>
          <Shield size={10} fill={item.role?.name === 'Super Admin' ? 'currentColor' : 'none'} />
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="relative group">
                <UserIcon size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300 group-focus-within:text-[#3C2F26] transition-colors" />
                <input
                  type="text"
                  required
                  value={formData.name}
            ...
            <div>
              <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Corporate Email</label>
              <div className="relative group">
                <Mail size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300 group-focus-within:text-[#3C2F26] transition-colors" />
                <input
                  type="email"
                  required
            ...
            <div>
              <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Division</label>
              <div className="relative group">
                <Building2 size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300 group-focus-within:text-[#3C2F26] transition-colors" />
                <input
                  type="text"
            ...
            <div>
              ...
              <div className="relative group">
                <Shield size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300 group-focus-within:text-[#3C2F26] transition-colors" />
                <select
                  required
            ...
            <div>
              <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">
                {editingId ? 'Credential Update (Optional)' : 'Security Credential'}
              </label>
              <div className="relative group">
                <Lock size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300 group-focus-within:text-[#3C2F26] transition-colors" />
                <input
                  type="password"
                  required={!editingId}
