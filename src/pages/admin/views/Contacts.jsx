...
const Contacts = () => {
  ...
  const getStatusBadge = (status) => {
    const configs = {
      new: { color: 'bg-blue-100 text-blue-600', icon: <Mail size={10} />, label: 'Incoming' },
      read: { color: 'bg-amber-100 text-amber-600', icon: <MailOpen size={10} />, label: 'Reviewed' },
      replied: { color: 'bg-emerald-100 text-emerald-600', icon: <Reply size={10} />, label: 'Responded' }
    };
    const config = configs[status] || { color: 'bg-gray-100 text-gray-400', icon: <Info size={10} />, label: status };
    
    return (
      <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${config.color}`}>
        {config.icon}
        {config.label}
      </span>
    );
  ...
  const columns = [
    {
      ...
      render: (item) => (
        <div className="flex flex-col">
          <span className={`font-bold ${item.status === 'new' ? 'text-[#3C2F26]' : 'text-gray-400'}`}>{item.name}</span>
    ...
    },
    {
      ...
      label: 'Snippet',
      render: (item) => (
        <p className={`text-xs truncate max-w-[200px] ${item.status === 'new' ? 'text-gray-600 font-medium' : 'text-gray-400'}`}>
          {item.message}
        </p>
      )
    },
          ...
          <div className="space-y-8">
            <div className="flex justify-between items-start border-b border-gray-50 pb-6">
              <div className="space-y-1">
                <h3 className="text-2xl font-black text-[#3C2F26] tracking-tight">{selectedContact.name}</h3>
                <div className="flex items-center gap-3 flex-wrap">
                  <span className="text-xs font-bold text-gray-400 bg-gray-50 px-2 py-1 rounded-lg uppercase tracking-wider">{selectedContact.email}</span>
                  {selectedContact.phone && (
                    <span className="text-xs font-bold text-gray-400 bg-gray-50 px-2 py-1 rounded-lg uppercase tracking-wider">{selectedContact.phone}</span>
                  )}
                </div>
              </div>
            ...
            {selectedContact.product && (
              <div className="flex items-center gap-3 p-4 bg-amber-50 border border-amber-100 rounded-xl">
                ...
                <div>
                  <p className="text-[10px] font-bold text-amber-600 uppercase tracking-wider">Inquiring About Product</p>
                  <p className="text-[#3C2F26] font-bold">{selectedContact.product.name}</p>
                </div>
              </div>
            )}
            ...
            <div className="relative">
              ...
              <div className="pl-6">
                <p className="text-[10px] font-black text-gray-300 uppercase tracking-[0.2em] mb-4">Transmission Payload</p>
            ...
            <div className="flex items-center gap-6 pt-6 border-t border-gray-50">
              <div className="flex-1 space-y-1">
                <p className="text-[10px] font-black text-gray-300 uppercase tracking-[0.2em]">Timestamp</p>
