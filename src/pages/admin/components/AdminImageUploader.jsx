...
export default function AdminImageUploader({
  ...
  const existingArr = Array.isArray(existingImages)
    ...
    : existingImages
      ? [existingImages]
      : [];
  const newArr = Array.isArray(newImages)
    ? newImages
    : newImages
      ? [newImages]
      : [];
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        {existingArr.map((img, idx) => (
          <div
            key={`existing-${idx}`}
            className="relative group aspect-square rounded-lg overflow-hidden border-2 border-blue-200"
          >
            <img
        ...
        {(multiple || (existingArr.length === 0 && newArr.length === 0)) && (
          <label className="flex flex-col items-center justify-center aspect-square border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-[#3C2F26] hover:bg-gray-50 transition drop-shadow-sm group">
            <ImagePlus
              size={24}
              className="text-gray-400 group-hover:text-[#3C2F26] transition"
            />
